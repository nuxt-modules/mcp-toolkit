...
The runtime is now [h3-mcp](https://mcp.h3.dev). `defineMcp*` still wrap plain returns and turn a throw into `isError`; transport, eras, auth, origin, and subscriptions come from the engine.
...
- `inputRequired` / `mcpElicit` / `getElicitedContent` replace `inputResponse` / `acceptedContent`.
