...
|--------|------|-------------|
| `maxAge` | `string \| number` | Cache duration (e.g., `'1h'`, `3600000`) |
| `getKey` | `(args) => string` | Custom cache key generator |
| `staleMaxAge` | `number` | Duration for stale-while-revalidate |
| `swr` | `boolean` | Enable stale-while-revalidate |
| `name` | `string` | Cache name (auto-generated from tool name) |
| `group` | `string` | Cache group (default: `'mcp'`) |
