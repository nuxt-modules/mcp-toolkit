    ...
    return {
      messages: [
        ...
        },
        {
          ...
          content: {
            type: 'text',
            text: 'Review the code I provide, checking for best practices and potential issues.',
          },
        },
      ],
    }
...
handler: async ({ code }) => ({
  messages: [{
    ...
    content: {
      type: 'text',
      text: `Review this code for Vue 3 best practices:\n\n${code}`,
    },
  }],
})
...
### 3. Combine Roles Effectively

Use multiple message roles to set up context properly:

```typescript
// Effective: System sets behavior, user provides task
messages: [
  { role: 'system', content: { type: 'text', text: 'You are an expert in accessibility.' } },
  { role: 'user', content: { type: 'text', text: 'Review this HTML for a11y issues.' } },
]
```
