...
seo:
  title: MCP Apps
  description: Author MCP UI Apps as Vue Single-File Components. Auto-bundled, served from a real Nuxt endpoint, and rendered inside Cursor, ChatGPT, Claude Desktop, Discord, and any MCP UI host.
---

## What are MCP Apps?

MCP Apps are **interactive HTML widgets** returned by an MCP tool and rendered inline by the host (Cursor, ChatGPT, Claude Desktop, Discord, …). Instead of streaming back text, your tool ships a small UI that the user can read, scroll, filter, and click — connected back to your server through a typed message bridge.
...
- Create a Vue SFC in app/mcp/ (e.g. app/mcp/stay-finder.vue)
...
2. Generates a **UI resource** at `ui://app/color-picker` exposing `text/html;profile=mcp-app`.
...
MCP Apps live in **`app/mcp/`** (not `server/mcp/`). They sit on the client side of Nuxt because they author Vue components — but the `handler` you declare runs server-side, just like a tool.

```bash
app/
└── mcp/
    ├── color-picker.vue    # → tool: color-picker, resource: ui://app/color-picker
    ├── stay-finder.vue     # → tool: stay-finder
    └── stay-checkout.vue   # → tool: stay-checkout
```

::callout{icon="i-lucide-info" color="info"}
Co-locate helpers next to the SFC (e.g. `stay-format.ts`) — the bundler inlines them. Keep data generation in `server/api/` and call it via `$fetch` from the handler.
::
...
| `stay-finder.vue` | `stay-finder` | `Stay Finder` |
...
defineMcpApp({
  description: 'Find boutique stays for a destination and date range.',
  inputSchema: {
    destination: z.string().describe('City or region, e.g. Lisbon'),
...
const { data, loading } = useMcpApp<StayPayload>()
</script>
```
...
Place shared types in Nuxt's `shared/` directory and import them through the `#shared` alias from both the SFC and your API endpoints:

```ts [shared/types/stays.ts]
export interface Stay { id: string, name: string, pricePerNight: number, image: string }
export interface StayPayload { query: StayQuery, stays: Stay[] }
export interface StayQuery { destination: string, checkIn: string, checkOut: string, travelers: number, nights: number }
```

```ts [server/api/stays.get.ts]
import { z } from 'zod'
import type { StayPayload } from '#shared/types/stays'

export default defineEventHandler(async (event): Promise<StayPayload> => {
  const query = await getValidatedQuery(event, QuerySchema.parse)
  return { query, stays: generateStays(query) }
})
```

The `import type` lines are stripped from the browser bundle by esbuild — the `#shared` alias never has to resolve inside the iframe.

## `useMcpApp()`

The single client-side composable, **auto-imported into every `app/mcp/*.vue` SFC**. It returns everything the iframe needs to talk to the host:
...
`data` is **already populated on first render** when the handler returns `structuredContent`. `loading` only flips to `true` while waiting for a `callTool()` refresh:

```vue
<template>
  <section v-if="loading" class="skeleton" />
  <section v-else-if="data" class="content">
    {{ data.stays.length }} stays in {{ data.query.destination }}
  </section>
</template>
```
...
```vue
<button @click="sendPrompt(`Book ${stay.name} for ${stay.travelers} guests`)">
  Reserve
</button>
```

In the playground, clicking **Reserve** in `stay-finder` triggers a follow-up that the LLM handles by calling the `stay-checkout` app. **App-to-app workflows** fall out of this primitive.

::callout{icon="i-lucide-triangle-alert" color="warning"}
Follow-ups are best-effort. Cursor and Claude Desktop forward the prompt cleanly. ChatGPT acknowledges the request but doesn't always re-render the next tool inline (an upstream limitation).
::
...
```vue
<script setup lang="ts">
const { data, pending, callTool } = useMcpApp<StayPayload>()

async function changeDates(checkIn: string, checkOut: string) {
  if (!data.value) return
  await callTool('stay-finder', { ...data.value.query, checkIn, checkOut })
}
</script>
```
...
```vue
<button @click="openLink(`https://example.com/stays/${stay.id}`)">
  View on the web
</button>
```
...
Origins must use `http(s)://` or `ws(s)://` and contain no path / query / quote characters — the toolkit rejects anything else at build time.

## How It's Wired

Behind the scenes, each `.vue` file under `app/mcp/` becomes **three artifacts** at build time:

1. A **tool definition** registered on your MCP handler (input schema, description, server handler).
2. A **UI resource** at `ui://app/<name>` returning `text/html;profile=mcp-app`.
...
2. The toolkit injects that data into the bundled HTML as `<script type="application/json" id="mcp-app-data">…</script>`.
...
|---|---|---|---|---|---|
| **Cursor** | ✅ | ✅ | ✅ | ✅ | Full support, including app-to-app via follow-ups. |
| **Claude Desktop** | ✅ | ✅ | ✅ | ✅ | Full support. |
| **ChatGPT (Apps SDK)** | ✅ | ⚠️ | ✅ | ✅ | Follow-ups are sent but the next tool isn't always rendered inline. |
| **Discord (MCP UI)** | ✅ | ✅ | ✅ | ➖ | External links open in browser when permitted. |
| **MCP Inspector** | ✅ | ✅ | ✅ | ➖ | Great for local development. |
...
```vue
<button :disabled="pending" @click="callTool('stay-finder', filters)">
  {{ pending ? 'Refreshing…' : 'Apply filters' }}
</button>
```
...
```vue
<!-- inside stay-finder.vue -->
<button @click="sendPrompt(`Check out ${stay.name} for €${stay.pricePerNight}/night.`)">
  Reserve
</button>
```

The LLM picks the tool name from the prompt — keep names predictable.
...
::callout{icon="i-lucide-info" color="info"}
Need rich components? Build a small co-located design system in `app/mcp/_components/` — it gets bundled into every app that imports it. Or pull in a headless library that doesn't depend on the host runtime.
::
