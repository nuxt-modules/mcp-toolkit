...
directory. Additional `vitePlugins` run between the toolkit's required Vue and single-file
plugins, which cannot be removed. Default exports listed in `vuePlugins` are installed on
the generated Vue app before it mounts. The mount node includes `isolate` by default.

For integrations that need more than Vue plugin installation, `entry` replaces the generated
mount entry. It cannot be combined with `vuePlugins`; import and install those plugins in the
