...
seo:
  title: Installation Guide - Nuxt MCP Toolkit
  description: Install and configure the Nuxt MCP module in your project.
---
...
Use the `nuxi` command to install the module and add it to your configuration automatically:
...
Don't forget to install `zod@^3` as well, as it is a required peer dependency.
::

#### Manual Installation

Install `nuxt-mcp-toolkit` and its peer dependency `zod`:

::code-group

```bash [pnpm]
pnpm add nuxt-mcp-toolkit zod@^3
```

```bash [npm]
npm install nuxt-mcp-toolkit zod@^3
```

```bash [yarn]
yarn add nuxt-mcp-toolkit zod@^3
```

```bash [bun]
bun add nuxt-mcp-toolkit zod@^3
```
