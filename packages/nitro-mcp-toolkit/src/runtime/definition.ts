 ...
 * options so the element types stay whatever it accepts. Each `defineMcp*` knows
 * which one it belongs in, so a definition is built by pushing itself.
 *
 * @internal
 */
...
export interface McpDefinition {
   ...
   * @internal
   */
  readonly build: (identity: McpIdentity, into: McpDefinitionBuckets) => void
}
